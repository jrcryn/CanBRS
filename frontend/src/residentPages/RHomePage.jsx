import React from 'react';
import { Link } from 'react-router-dom';
import ReservationProcess from '../components/ReservationProcess';
import ReservationStep from '../components/ReservationStep';

import { Button, Flex, Heading, Image, Stack, Text, useBreakpointValue, Box
} from '@chakra-ui/react'

function HomePage() {
  return (
    <>
    <Stack minH={'95vh'} direction={{ base: 'column', md: 'row' }}>
      <Flex p={8} flex={1} align={'center'} justify={'center'}>
        <Stack spacing={6} w={'full'} maxW={'lg'}>
          <Heading fontSize={{ base: '3xl', md: '4xl', lg: '5xl' }}>
          <Text
            as={'span'}
            fontSize={useBreakpointValue({ base: 'md', md: 'lg', lg: '6xl' })} 
            position={'relative'}
            _after={{
                content: "''",
                width: 'full',
                height: useBreakpointValue({ base: '20%', md: '30%' }),
                position: 'absolute',
                bottom: 1,
                left: 0,
                bg: 'blue.400',
                zIndex: -1,
            }}>
            CanBRS
            </Text>
            <br />{' '}
            <Text color={'blue.500'} as={'span'}>
              Canlubang Booking and Reservation System 
            </Text>{' '}
          </Heading>
          <Text fontSize={{ base: 'md', lg: 'lg' }} color={'gray.500'}>
          A digital booking and information viewing platform for the community resources of Barangay Canlubang.
          </Text>
          <Stack direction={{ base: 'column', md: 'row' }} spacing={4}>
            <Button
              as={Link} 
              to={'/resident/listing'}
              rounded={'full'}
              bg={'blue.500'}
              color={'white'}
              _hover={{
                bg: 'red.500',
              }}>
              See Availabilities
            </Button>
            <Button rounded={'full'} as={Link} to={'/resident/urgent-needs'}>For Urgent Needs</Button>
          </Stack>
        </Stack>
      </Flex>
      <Flex flex={1}>
        <Image
          alt={'Login Image'}
          objectFit={'cover'}
          src={
            'https://images.unsplash.com/photo-1527689368864-3a821dbccc34?ixid=MXwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHw%3D&ixlib=rb-1.2.1&auto=format&fit=crop&w=1350&q=80'
          }
        />
      </Flex>
    </Stack>
    <Box mt={100}> {/* Adjust the margin-top value as needed */}
        <ReservationProcess />
    </Box>
    <Box mt={10}> {/* Adjust the margin-top value as needed */}
        <ReservationStep />
    </Box>
    </>
  )
}

export default HomePage